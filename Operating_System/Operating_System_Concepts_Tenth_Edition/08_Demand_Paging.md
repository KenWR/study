...
이러한 경우 메모리에 올라와 있는 페이지와  보조 메모리에 있는 페이지간의 교체가 필요하다

동작 방식은 다음과 같다

1. 교체대상(victim) 을 선택한다
2. 교체대상 제거(page out)   
   1. 교체대상의 [dirty bit](#dirty-bit)가 dirty하다면 보조 메모리에 write   
3. 필요한 페이지를 보조 메모리로부터 가져온다(page in)   
4. 페이지 테이블을 갱신시킨다   

여기서 우리가 당연히 가져야하는 질문은 `페이지 교체를 할 때, 교체대상은 어떻게 정해지는가?` 이다   

교체대상을 정하는 알고리즘은 대표적으로 4가지 정도가 있다   
...
페이지를 새로 읽어들일 때, FIFO 방식을 사용하여 버퍼와 교체대상을 선택하여 새로운 페이지를 할당하고 교체대상을 버퍼에 저장한다   
그리고 빈 프레임 풀의 내용을 초기화 하지않고 만약 빈 프레임 목록에 있는 프레임중 하나가 재참조되면 바로 그대로 꺼내서 쓸 수 있게 한다   
(보조메모리가 유휴상태일 때, dirty한 페이지를 write함)   
...
dirty bit를 추가한것
보조 메모리의 I/O를 줄이기위해 그래도 가장 우선순위애서 I/O를 안해도 되는 놈을 고르는 것   
...
일반적으로 프레임 수가 높아지면 페이지 폴트가 적어지는 것으로 예상되지만 FIFO에서는 오히려 페이지 폴트 수가 늘어나는 현상   

### [Thrashing]

CPU scheduler 와 Memory Management 사이에서 발생하는 괴리   

CPU utilization이 낮으면 추가적인 프로세스들을 투입하는 역할을 CPU 스케줄러가 하는데 문제는 메모리에 상주하는 프로세스들이 너무 많아짐에 따라 페이지 폴트가 많이 발생하게되고 그로인한 문맥교환이나 보조 메모리 I/O가 많이 발생함으로서 실행중인 프로세스들이 대부분 대기상태로 들어가면서 CPU사용량이 줄어들게되는데 이를 본 CPU스케줄러가 추가적인 프로세스들을 투입하게되어 process가 너무 과다해지고 처음부터 다시 똑같은 악순환이 발생하게 되는것   
...
메모리 블록이 교체되거나 제거되는 경우 해당 비트를 참조하여 해당 블록을 보조 메모리에 write할지 여부를 결정한다   

*왜 이런식으로 동작하는가?*  

CPU와 가장 멀리 떨어져있는 보조메모리에 대한 I/O 비용은 굉장히 비싸다   
